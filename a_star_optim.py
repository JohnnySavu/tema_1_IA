...
def a_star(gr, nrSolutiiCautate, timeout, output_path, heuristic = 'banala'):
    ...
    l_open = [NodParcurgere(gr.start, 0, None, gr.calculeaza_h(gr.start, heuristic))]
    ...
    while len(l_open) > 0:
        ...
        lSuccesori = gr.genereazaSuccesori(nodCurent, heuristic)
        for s in lSuccesori:
            gasitC = False
            amSters = False
            for nodC in l_open:
        ...
        for s in lSuccesori:
            ...
            if gasit_loc:
                ...
                l_open.append(s)
        max_in_memory = max(max_in_memory, len(l_open))
...
if __name__ == "__main__":
    ...
    output_name_bias = "_a_star_opt_" + (inputFile.split(".")[0].split('/'))[1] + "_" + euristic_type
